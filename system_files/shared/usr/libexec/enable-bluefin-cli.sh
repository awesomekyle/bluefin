...
function get_status(){
    if systemctl --quiet --user is-enabled bluefin-dx-cli.target; then
        bluefin_cli[0]="${green}Enabled${n}"
    else
        bluefin_cli[0]="${red}Disabled${n}"
    fi
    if systemctl --quiet --user is-active bluefin-dx-cli.service; then
...
function logic(){
    if test "$toggle" = "Enable"; then
        ...
        systemctl --user enable --now bluefin-dx-cli.target > /dev/null 2>&1
        if ! systemctl --quiet --user is-active bluefin-dx-cli.service; then
            systemctl --user reset-failed bluefin-dx-cli.service > /dev/null 2>&1 || true
            echo "${b}${green}Starting${n} Bluefin-CLI"
            systemctl --user start bluefin-dx-cli.service
        fi
        default_login
    elif test "$toggle" = "Disable"; then
        echo "${b}${red}Disabling${n} Bluefin-CLI"
        systemctl --user disable --now bluefin-dx-cli.target > /dev/null 2>&1
        if systemctl --quiet --user is-active bluefin-dx-cli.service; then
            echo "Do you want to ${b}${red}Stop${n} the Container?"
            stop=$(Confirm)
            if test "$stop" -eq 0; then
                systemctl --user stop bluefin-dx-cli.service > /dev/null 2>&1
                systemctl --user reset-failed bluefin-dx-cli.service > /dev/null 2>&1 || true
            fi
        fi
        echo "Setting Host back to default Ptyxis Profile"
